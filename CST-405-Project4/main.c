...
static const char* preprocessModulo(const char* inputPath) {
    ...
    int hasModulo = 0;
    for (long i = 0; i < sz; i++) {
        /* skip string literals and char literals to avoid false matches */
        if (src[i] == '%') { hasModulo = 1; break; }
    }

    if (!hasModulo) {
        free(src);
        return inputPath;   /* no % in source, skip preprocessing */
    }

    printf("    [PREPROCESSOR] Found %% operator — injecting __mod helper and rewriting\n");
